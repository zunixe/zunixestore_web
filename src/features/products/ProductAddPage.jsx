...
export default function ProductAddPage() {
    ...
    await dispatch(createProduct({
      ...
      data: {
        ...
        description: JSON.stringify({
          text: form.description, short_desc: form.short_desc, barcode: form.barcode,
          ...
          shipping_time: Number(form.shipping_time), shipping_unit: form.shipping_unit,
          custom_label: form.custom_label, availability: form.availability,
          out_of_stock: form.out_of_stock, digital_product: form.digital_product,
          interest_count: form.interest_count, audience: form.audience,
          schedule_display: form.schedule_display, schedule_launch: form.schedule_launch,
        }),
      },
    }));
    setSaving(false);
  ...
  const sectionCard = 'bg-white rounded-xl border border-gray-100 p-5';
    ...
    return (
      <div className={sectionCard}>
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={() => setOpen(!open)} className="flex items-center gap-1 text-sm text-blue-600 font-medium hover:text-blue-700">
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="grid grid-cols-[1fr_400px] gap-5">
        ...
        <div className="space-y-5">
          ...
          {/* Informasi Produk */}
          <div className={sectionCard}>
            <h3 className={`${sectionTitle} mb-4`}>Informasi Produk</h3>
          ...
          {/* Manajemen Produk */}
          <div className={sectionCard}>
            <h3 className={`${sectionTitle} mb-4`}>Manajemen Produk</h3>
            <div className="space-y-4">
        ...
        <div className="space-y-5">
          {/* Detail Produk */}
          <div className={sectionCard}>
            <h3 className={`${sectionTitle} mb-4`}>Detail Produk</h3>
            <div className="space-y-4">
              <div className="flex items-center gap-2">
          ...
          {/* Varian */}
          <div className={sectionCard}>
            <h3 className={`${sectionTitle} mb-3`}>Varian</h3>
          ...
          {showMore && (
            <>
              <CollapsiblePanel title="Pengelompokkan">
                <p className="text-xs text-gray-500">Kelompokkan produk ini dengan produk lain.</p>
              </CollapsiblePanel>
              <CollapsiblePanel title="Panduan ukuran produk">
                <p className="text-xs text-gray-500">Tambahkan panduan ukuran untuk produk ini.</p>
              </CollapsiblePanel>
              <CollapsiblePanel title="Grosir">
                <p className="text-xs text-gray-500">Atur harga grosir untuk pembelian dalam jumlah banyak.</p>
              </CollapsiblePanel>
              <CollapsiblePanel title="Catatan Produk">
                <p className="text-xs text-gray-500">Tambahkan catatan internal untuk produk ini.</p>
              </CollapsiblePanel>
              <CollapsiblePanel title="Down payment (DP)">
                <p className="text-xs text-gray-500">Atur sistem DP untuk produk ini.</p>
              </CollapsiblePanel>
              <CollapsiblePanel title="Pengiriman">
                <p className="text-xs text-gray-500">Atur pengiriman khusus untuk produk ini.</p>
              </CollapsiblePanel>
            </>
          )}

          <button type="button" onClick={() => setShowMore(!showMore)} className="flex items-center gap-2 text-sm text-blue-600 font-medium hover:text-blue-700">
            {showMore ? 'Tampilkan Sebagian Pilihan Saja' : 'Lihat pilihan lain'}
